...
def get_args(args=None, parse_known=False):
    ...
    parser.add_argument('--ng_update',
                        type=int,
                        default=200000,
                        help='number of G update to train for')
    parser.add_argument('--n_dstep',
                        type=int,
                        default=1,
                        help='number of D update step')
...
def gan_bridge(hparams):
    ...
    if opt.g_type == 'sheldon_mlp':

        netG = sheldonMLPGenerator(hdim=opt.ngf,
                                   img_dims=img_dims,
                                   zdim=opt.nz,
        ...
        netG = sheldonMLPGenerator2(hdim=opt.ngf,
                                    img_dims=img_dims,
                                    zdim=opt.nz,
                                    spectral_norm=opt.g_sn,
                                    flat_out=False).to(device)
        ...
        netG = sheldonMLPGenerator3(hdim=opt.ngf,
                                    img_dims=img_dims,
                                    zdim=opt.nz,
                                    spectral_norm=opt.g_sn,
                                    flat_out=False).to(device)
    ...
    gan_image_dir = hparams.messenger.readable_dir + '/samples.png'
