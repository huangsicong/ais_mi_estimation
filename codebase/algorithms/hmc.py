...
def accept_reject(current_z, batch, t, current_v, hmc_z, hmc_v, epsilon,
    ...
    with torch.no_grad():
        ...
        uniform_sample = torch.rand(prob.size()).to(device=hparams.device)
        accept = (prob > uniform_sample).type(
            hparams.dtype).to(device=hparams.device)
        ...
        if hparams.step_sizes_target is None:
            criteria = (accept_hist / hist_len >
                        task_params.acceptance_prob).type(
                            hparams.dtype).to(device=hparams.device)
